...
class InceptionLayer(LayerInterface):
    ...
    def __init__(self, amount_of_filters: int, filter_size: Optional[int] = None, used_dil_values: Optional[List[int]] = None, used_dil_ratios: Optional[List[float]] = None, name: str = ""):
        ...
        if filter_size is None:
            filter_size = LayerInterface.settings("Model/normal_filter_size")
        ...
        if used_dil_values is None:
            ...
            used_dil_values = LayerInterface.settings("Model/InceptionLayer/used_dilation_values")[:amount_of_filters]
        if used_dil_ratios is None:
            # to make sure only a certain amount of filters is used
            used_dil_ratios = LayerInterface.settings("Model/InceptionLayer/used_dilation_ratios")[:amount_of_filters]
        ...
        if LayerInterface.settings("Model/InceptionLayer/regularizer_scale") > 1e-13:
            kernel_regularizer = tf.keras.regularizers.L2(LayerInterface.settings("Model/InceptionLayer/regularizer_scale"))
