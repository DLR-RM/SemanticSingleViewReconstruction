...
public:
	...
	Space(const uiPoint& iSize, const dPoint& origin, const dPoint& size): m_data(iSize), m_class(iSize), m_origin(origin), m_size(size) {};
    ...
    Array3D<int>& getObjClassData(){ return m_class; }

	static double interCalcDistExactlyForPoint(const dPoint &point, const Octree &octree, std::vector<QueueElement> &openList, std::vector<const Octree *> &neighbours, std::vector<bool> &visitedNodes, const Polygon *&closestPoly, double maxDistanceToMinPosDist);
	static Octree createOctree(const Polygons& polys, double minimumOctreeVoxelSize);

    dPoint getCenterOf(unsigned int i, unsigned int j, unsigned int k);

    bool isPointInSpace(const iPoint& point);
