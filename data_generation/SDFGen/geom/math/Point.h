...
public:
    ...
    void clipToMaxAbsValue(const double maxAbsValue){
        for(int i = 0; i < 3; ++i){
            if(fabs(m_data[i]) > maxAbsValue){
                m_data[i] = copysign(maxAbsValue, m_data[i]);
            }
        }
    }
...
public:
	...
	Point3D(Point<double> rhs) : Point<double>(rhs[0], rhs[1], rhs[2]), m_index(0), m_used(false){};
