...
def predict_for_file(single_file_path: Union[str, Path], output_file_path: Union[str, Path],
    ...
    start_amount_of_files = time.time()
    for current_iteration in range(amount_of_iterations):
        ...
        for points_input, output_tsdf, classes, ids, latent_vector in sorted_dataset:
            ...
            if len(trainer.latent_optimizer.get_weights()) != 3:
    ...
    with h5py.File(output_file_path, "w") as file:
        ...
        is_non_boundary = str(True)
        file.create_dataset("is_non_boundary", data=np.string_(is_non_boundary), dtype=np.string_(is_non_boundary).dtype)
    ...
    print(f"{time.time() - start_amount_of_files}s, {total_used_latent_vars}, {(time.time() - start_amount_of_files) / total_used_latent_vars}")
