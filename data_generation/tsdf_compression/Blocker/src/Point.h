...
public:
    ...
    Point<dataType> floorThis(){
        m_data[0] = std::floor(m_data[0]);
        m_data[1] = std::floor(m_data[1]);
        m_data[2] = std::floor(m_data[2]);
        return *this;
    }

	// transform contains a transformation matrix
    template<typename T>
	void transformPoint(const T& transform){
	    const double x = m_data[0] * transform[0] + m_data[1] * transform[4] + m_data[2] * transform[8] + transform[12];
        const double y = m_data[0] * transform[1] + m_data[1] * transform[5] + m_data[2] * transform[9] + transform[13];
        const double z = m_data[0] * transform[2] + m_data[1] * transform[6] + m_data[2] * transform[10] + transform[14];
        m_data[0] = x;
        m_data[1] = y;
        m_data[2] = z;
    }
...
public:
	...
	Point3D() : Point(), m_class(0), m_distance(0), m_batchCounter(-1){};

	Point3D(double x, double y, double z, unsigned int classId, double distance, int batchCounter) : Point<double>(x, y, z), m_class(classId), m_distance(distance), m_batchCounter(batchCounter){};

    unsigned int getClass() const {return m_class; }
    double getDistance() const {return m_distance; }
    int getBatchCounter() const {return m_batchCounter; }
...
template<typename dataType>
static dPoint pointFloor(const Point<dataType>& lhs){
    return {floor(lhs[0]), floor(lhs[1]), floor(lhs[2])};
}
