...
def load_front3d(json_path: str, future_model_path: str, front_3D_texture_path: str, label_mapping: LabelIdMapping,
                 ...
                 front_3D_obj_mesh_folder: str = None) -> List[MeshObject]:
    """ Loads the 3D-Front scene specified by the given json file.
    This also creates the wall object
    ...
    for obj in created_objects:
        if obj.get_cp("category_id") == 51:
            location = obj.get_location()
            location[2] += 0.001
            obj.set_location(location)
            floors.append(obj)
    height_var = np.var([np.min(obj.get_bound_box(), axis=0)[2] for obj in floors])
    height_mean = np.mean([np.min(obj.get_bound_box(), axis=0)[2] for obj in floors])
    if height_var > 0.0:
        raise Exception("This file can not been used not all floors are on the same level")

    all_custom_properties = set()
    for obj in created_objects:
        all_custom_properties.add(obj.get_cp("category_id"))
        if obj.get_cp("category_id") == 79:
            height_location = np.mean(obj.get_bound_box(), axis=0)
            location = obj.get_location()
            location[2] = height_mean - 0.01 - height_location[2]
            obj.set_location(location)
    ...
    used_objects = []
    for category_id in all_custom_properties:
        found_object = False
        for obj in created_objects:
            if obj in used_objects:
                continue
            if category_id == obj.get_cp("category_id"):
                used_objects.append(obj)
                if "." not in obj.get_name():
                    found_object = True
        ...
        if found_object:
            ...
            print("No start object could be found!")
            exit(1)
    created_objects = [obj for obj in get_all_mesh_objects() if obj.has_cp("is_3d_front") and obj.get_cp("is_3d_front")]
    # flip certain obj types
    flipped_category_ids = [4, 7, 21, 30, 38, 40, 51, 58, 75, 93, 95, 98]
    bpy.ops.object.select_all(action='DESELECT')
    for obj in created_objects:
        if obj.get_cp("category_id") in flipped_category_ids:
            obj.select()
            bpy.context.view_layer.objects.active = obj.blender_obj
            obj.edit_mode()
    ...
    remove_object_ids = [1, 37, 43, 45, 52, 54, 75, 85, 90, 97]
    while True:
        found_one = False
        for obj in created_objects:
            if obj.get_cp("category_id") in remove_object_ids:
                obj.delete()
    ...
    if front_3D_obj_mesh_folder is not None:
        # save the wall objects
        bpy.ops.object.select_all(action='DESELECT')
        for obj in created_objects:
            obj.select()
        obj_folder = os.path.join(front_3D_obj_mesh_folder, os.path.basename(json_path.replace(".json", "")))
        if not os.path.exists(obj_folder):
            os.makedirs(obj_folder)
        obj_file_path = os.path.join(obj_folder, "wall_objects.obj")
        print("Save the walls to this {}".format(obj_file_path))
        bpy.ops.export_scene.obj(filepath=obj_file_path, use_selection=True, use_triangles=True, path_mode="ABSOLUTE",
                                 use_materials=False, use_uvs=False, use_normals=False)
        for obj in created_objects:
            obj.deselect()
        ...
        def do_line(line):
            if line.startswith("o "):
                ...
                for pos in range(3, len(line)):
                    label_name = line[2: pos].lower()
                    if label_mapping.has_label(label_name):
                        return "o " + str(label_mapping.id_from_label(label_name))
                if not not_found:
                    raise Exception(f"This warning: {line}")
            return line

        text = [do_line(line) for line in text]
    ...
    all_used_loaded_furniture = Front3DLoader._move_and_duplicate_furniture(data, all_loaded_furniture)

    if front_3D_obj_mesh_folder is not None:
        text = []
        # remove added blender rotation
        blender_rot_mat = np.array(mathutils.Matrix.Rotation(radians(-90), 4, 'X'))

        for obj in all_used_loaded_furniture:
            world_2_mat = obj.get_local2world_mat()
            world_2_mat = blender_rot_mat @ world_2_mat
            world_2_mat = world_2_mat.reshape(-1)
            text.append(
                " ".join([str(e) for e in world_2_mat]) + " " + os.path.abspath(obj.get_cp("file_path")) + " " + str(
                    obj.get_cp("category_id")))
        # remove doubles -> as some obj files contain multiple objs
        text = list(set(text))
        with open(os.path.join(obj_folder, "position.txt"), "w") as file:
            file.write("\n".join(text))

    # add an identifier to the obj
    for obj in all_used_loaded_furniture:
        obj.set_cp("is_3d_front", True)
    created_objects += all_used_loaded_furniture
...
class Front3DLoader:
                ...
                if used_mat["texture"]:
                    ...
                    if hash_folder in used_materials_based_on_texture and "ceiling" not in used_obj_name.lower():
                        ...
                        if "ceiling" in used_obj_name.lower():
                            ...
                            mat.set_name(mat.get_name() + "_emission")
    ...
    def _load_furniture_objs(data: dict, future_model_path: str, lamp_light_strength: float,
        ...
        all_objs = []
        load_correction_paths = os.path.join(future_model_path, "category_mapping.json")
        if os.path.exists(load_correction_paths):
            with open(load_correction_paths, "r") as file:
                load_corrections = json.load(file)
        else:
            raise Exception("The category mapping file must exist: {}".format(load_correction_paths))
        ...
        for ele in data["furniture"]:
            ...
            used_obj_file = [path for path in [os.path.join(folder_path, file_name) for file_name in
                                               ["raw_model.obj", "raw_model.blend"]] if os.path.exists(path)]
            if len(used_obj_file) > 0:
                obj_file = used_obj_file[-1]
            else:
                continue
            ...
            if os.path.exists(obj_file):
                is_blend_file = obj_file.endswith(".blend")
                if is_blend_file:
                    objs = load_blend(path=obj_file, obj_types=["mesh"])
                else:
                ...
                for obj in objs:
                    if ele["jid"] in load_corrections:
                        used_obj_name = load_corrections[ele["jid"]]
                    obj.set_name(used_obj_name)
                    # add some custom properties
                    obj.set_cp("uid", ele["uid"])
                    obj.set_cp("jid", ele["jid"])
                    obj.set_cp("is_blend_file", is_blend_file)
    ...
    def _move_and_duplicate_furniture(data: dict, all_loaded_furniture: list) -> List[MeshObject]:
        ...
        for obj in all_loaded_furniture:
            if obj.get_cp("uid") not in objs_used:
                obj.delete()
        return created_objects
